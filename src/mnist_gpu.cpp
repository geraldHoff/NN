...
int main() {
    std::cout << "=== MNIST Digit Classification (GPU) ===\n\n";
    
    // Load dataset (use subset for faster training)
    std::cout << "Loading MNIST dataset...\n";
    auto train_images = read_mnist_images("train-images-idx3-ubyte", 5000);  // 5000 training
    auto train_labels = read_mnist_labels("train-labels-idx1-ubyte", 5000);
    auto test_images = read_mnist_images("t10k-images-idx3-ubyte", 1000);   // 1000 test
    ...
    // Create GPU neural network
    // Architecture: 784 inputs -> 128 hidden -> 64 hidden -> 10 outputs
    std::cout << "Creating GPU neural network (784 -> 128 -> 64 -> 10)...\n\n";
    Model model;
    model.add(std::make_unique<DenseGPU>(784, 128));
    model.add(std::make_unique<ReLUGPU>());
    model.add(std::make_unique<DenseGPU>(128, 64));
    model.add(std::make_unique<ReLUGPU>());
    model.add(std::make_unique<DenseGPU>(64, 10));
    
    MSELossGPU loss_fn;  // GPU loss function
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t i = 0; i < train_images.size(); ++i) {
            ...
            // Update weights (THIS IS THE TRAINING STEP!)
        ...
        for (size_t i = 0; i < test_images.size(); ++i) {
            ...
            // Find predicted digit (index of max output)
            int pred_digit = std::max_element(pred.data.begin(), pred.data.end()) - pred.data.begin();
            
            // Find true digit (index of 1.0 in one-hot encoding)
